...
def make_conv(example, model_type, prev_output=None):
    # Prompt template copied from: https://github.com/mandyyyyii/scieval/blob/main/eval/eval_zeroCot.py#L11
    prompt_round1 = example['cot_prompot'] + '\n' + example['question'] + "\nLet's think step by step: "
    prompt_round2 = ' Therefore, the answer is'
...
if __name__ == "__main__":
    ...
    test_set = load_dataset('OpenDFM/SciEval')['valid'].to_pandas()
    test_set["prompt_round1"] = test_set.apply(lambda example: make_conv(example, args.model_type), axis=1)
    
    # Adapted from: https://github.com/OpenDFM/SciEval/blob/main/eval.py
    ...
    test_set.to_json(os.path.join(args.save_dir, "scieval_completion.json"))
    
    answered_set = dict()
    correct, wrong = 0, 0

    output_filename = os.path.join(args.save_dir, "scieval_output.json")
    ...
    for example in test_set.to_dict(orient="records"):
        ...
        verifier = SciEvalTask(id=example['id'], 
